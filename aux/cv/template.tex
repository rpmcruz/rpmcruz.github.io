...
& & \textcolor{gray}{ {{ item.university }} } \\
{% endfor %}
\end{tabular}

\begin{tikzpicture}[remember picture, overlay]
{% for item in education %}
\draw[dashed] let \p1=(pic cs:e{{ loop.index }}) in ({2mm,\y1+1mm}) -- ++ (2.5mm,0);
{% endfor %}
...
& & \textcolor{gray}{ {{ item.where }} } \\
{% endfor %}
\end{tabular}

\begin{tikzpicture}[remember picture, overlay]
{% for item in career %}
\draw[dashed] let \p1=(pic cs:c{{ loop.index }}) in ({2mm,\y1+1mm}) -- ++ (2.5mm,0);
{% endfor %}
...
\begin{tikzpicture}[remember picture, overlay]
{% for item in awards %}
\draw[dashed] let \p1=(pic cs:a{{ loop.index }}) in ({2mm,\y1+1mm}) -- ++ (2.5mm,0);
{% endfor %}
...
& & \textcolor{gray}{ {{ item.authors }} } \\
& & \textcolor{gray}{ {{ item.publisher|replace('&', '\\&') }} } \\
{% endfor %}
\end{tabular}

\begin{tikzpicture}[remember picture, overlay]
{% for item in publications %}
\draw[dashed] let \p1=(pic cs:p{{ loop.index }}) in ({2mm,\y1+1mm}) -- ++ (2.5mm,0);
{% endfor %}
